...
一般走道每步：`2 * odom.twist.twist.linear.x - abs(balance_m) - 0.05`。
片段最後一步已到開口時不扣置中分。碰撞代理判定優先，總獎勵直接為 −100。
無目標方向、路徑點進度或完成路線獎勵。完整巡邏靠狀態機保證任務順序，
不能用局部累積獎勵代替全程成功率。
...
| `open_distance` / `wall_distance` | 1.5 / 1.0 m | 分離開口與走道壁閾值 |
...
| `collision_margin` / `stop_margin` | 0.02 / 0.18 m | 碰撞代理及前進停止裕量 |
