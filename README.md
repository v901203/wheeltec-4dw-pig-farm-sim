...
完整巡邏入口為 `rl_pig_pen/fsm_lidar_patrol.py`，以傳統控制完成左支線前進到底、倒退穿過路口至右端、前進回中心、轉回主幹道並繼續下一個路口的循環，直到主幹道終點。原本直行腳本維持不變。

完成設定的 3 個路口後，進入 `FINAL_MAIN`，只沿主幹道行駛並確認終點牆，不再接受第四個支線入口；確認終點後才停止並回報 `DONE`。

截至 2026-09-23，使用者已回報完成 Gazebo 路口巡邏測試，最近一輪自動回歸測試 33 項通過。整趟完成的驗收條件是 `DONE`、3 個完成路口、6 個已到達端點及終點停車；路口完成回報尚不代表已建立重複成功率或完成實車驗收。
...
| 支線巡邏、倒車、路口轉 90° | 已實作；使用者回報 Gazebo 路口測試完成 |
| 主幹道末端與禁止第四個支線入口 | 已實作，相關自動測試通過；現場驗收需確認最終 `DONE` |
| 遮擋追蹤、越位回正與辨識跳變處理 | 已實作並有回歸測試，仍需重複運行統計 |
| 傳統控制＋RL 混合架構 | 尚未實作 |
...
收到第一筆掃描後，車子便會開始控制。預設巡航速度為 0.2 m/s，直行修正角速度上限為 ±0.3 rad/s；前方 ±10° 內的最短距離小於 0.55 m 時停車，障礙離開後會恢復前進。
...
目前優先處理的問題包括：純左右偏移可能讓路口置中停滯、單一無效雷達點可能導致鎖定停車，以及實車需要獨立於上層控制程序的指令逾時保護。前兩項已用隔離診斷重現；詳見 [檢查結果與後續工作](rl_pig_pen/README.md#檢查發現與待改善項目)。本次檢查記錄問題，尚未實作相應修正。
...
實體部署時，LiDAR 驅動、速度介面、指令逾時停車與急停需要分別確認。現有串口轉接腳本會重送最後收到的速度，不能假設上層失聯便會自動停車；底盤端是否已有獨立保護仍需確認。

下一階段先改善已重現的控制邊界問題，再記錄多回合的成功率、完成時間及失敗原因，逐步驗證滑動、延遲與動態障礙的影響，之後再進行 Xavier／實車測試。
