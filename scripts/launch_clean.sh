...
if [[ -d "$PIG_MODEL_PKG_DIR" ]]; then
	# Fortress (ign gazebo) 主要吃 IGN_GAZEBO_RESOURCE_PATH，同時保留 GZ_SIM_RESOURCE_PATH 以防萬一
	...
	echo "pig_model 資源路徑已加入 IGN_GAZEBO_RESOURCE_PATH / GZ_SIM_RESOURCE_PATH: $PIG_MODEL_PKG_DIR"
else
...
echo "Starting ign gazebo server (headless)..."
ign gazebo -s -r "$WORLD_FILE" &> "$LOG_DIR/gz_sim_server.log" &
GZ_PID=$!
sleep 3

echo "Starting ign gazebo GUI (attaching to running server)..."
# 注意：故意不用 `ign gazebo <world> -r`（server+GUI 一起啟動），
